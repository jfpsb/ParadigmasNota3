...
public class SessaoManager {
	...
	/**
	 * Cria uma entidade de sessao, se não existirem conflitos de horario e
	 * sala, a sessão será criada e persistida no banco.
	 * 
	 * @param sala
	 *            Referência da Sala.
	 * @param filme
	 ...
	 */
	public static boolean criarSessão(Sala sala, Filme filme, LocalDateTime data, boolean isLegendado, boolean is3d,
			double preco) {
	...
	/**
	 * Atualiza uma sessao sessao, se não existirem conflitos de horario e sala,
	 * a sessão será criada e persistida no banco.
	 * 
	 * @param sessao
	 *            Sessao a ser atualizada
	 * @param sala
	 *            Referência da Sala.
	 * @param filme
	 *            Referência do Filme.
	 ...
	 * @return Retorna verdade se a entidade for criada.
	 */
	public static boolean atualizarSessão(Sessao sessao, Sala sala, Filme filme, LocalDateTime data,
			boolean isLegendado, boolean is3d, double preco) {
	...
	/**
	 * Remove uma sessao do banco, caso não existam reservas. Se o parametro de
	 * ignorar estiver verdadeiro as reservas subsequentes serão removidas.
	 * 
	 * @param sessao
	 *            Sessao a ser removida.
	 * @param ignorarReservas
	 *            Booleana para ignorar reservas.
	...
	public static List<Sessao> listarSessao() {
		if (sessoeslistChanged)
			sessoes = daoSessao.listar();
	...
	public static List<Sessao> listarSessaoPorHorario(LocalDateTime data) {
		...
		for (Sessao sessao : listarSessao()) {
			...
			if ((sessao.getData().isBefore(data) && dataEnd.isAfter(data)) || sessao.getData().isEqual(data)
					|| dataEnd.isEqual(data)) {
				aux.add(sessao);
			}
		}
		return aux;
	 ...
	 * 
	 * @param sessao
	 *            Sessao a qual a reserva vai ser feita
	 * @param ingresso
	 *            Ingresso representativo a ser associada a reserva
	 * @param coluna
	 *            Coluna da reserva que deseja
	 * @param linha
	 *            Linha da reserva que deseja
	...
	public static boolean reservarPoltrona(Sessao sessao, Ingresso ingresso, int coluna, int linha) {
		...
		for (Reserva reserva : listarReservasDaSessao(sessao)) {
			if (reserva.getLinha() == linha && reserva.getColuna() == coluna) {
	...
	public static List<Ingresso> listarIngressosDaSessao(Sessao sessao) {
		...
		for (Ingresso ingresso : listarIngresso()) {
	 ...
	 * 
	 * @return Lista com todos os ingressos.
	 */
	public static List<Ingresso> listarIngresso() {
		return daoIngresso.listar();
	}
